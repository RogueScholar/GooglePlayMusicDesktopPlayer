...
function mprisService() {
  ...
  getPosition() { return PlaybackAPI.currentTime().current * 1e3; };
  ...
  PlaybackAPI.on('change:state', (playbackState) => {
    // DEV: We skip stopped here because PlaybackAPI only shuttles true/false
    // from GPM
    player.playbackStatus = (playbackState) ? 'Playing' : 'Paused';
  });

  PlaybackAPI.on('change:volume',
                 (volume) => { player.volume = volume / 100; });
}
